...
diary('off');
log_filename= sprintf('./log/SSC2_n_%02d.txt',nn);
delete(log_filename);
diary(log_filename);
fprintf('SRP-PHAT (TL-SSC) (n=%d)\n',nn);
time= clock;
fprintf('%s, %02d:%02d\n',date,time(4),time(5));
...
for i=1:nfile
    ...
    ans_r= str2num(vadframe_filename(26)) * 1000;
    ans_height= 1700 - 400;
    ans_phi= (rad2deg(atan2(ans_height,ans_r)));
    ans_theta= str2num(vadframe_filename(22:24));
    if (DEBUG==1)
    ...
    for f=1:nFrame
        ...
        I= srp_phat_tlssc(input_frames,TDOA_table_SSC,TDOA_table_SSC2,SSC2,nn);
        
        [t,p,r]= cart2sph(SSC_centroids(I,1), ...
                          SSC_centroids(I,2), ...
                          SSC_centroids(I,3));
